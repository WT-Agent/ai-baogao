<template>
  <div class="app-container">
    ...
    <header>
      ...
      <h1>{{ appTitle }}</h1>
      <p>高校社会实践 · 实验实训总结 · 岗位实习报告 · 课堂研讨心得</p>
    </header>
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="selector-group">
        <label class="selector-label">选择报告类型</label>
        <div class="style-selector">
          <button 
            v-for="rtype in reportTypeOptions" 
            :key="rtype.value"
            class="style-option"
            :class="{ active: activeReportType === rtype.value }"
            @click="activeReportType = rtype.value"
          >
            {{ rtype.label }}
          </button>
        </div>
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">所属专业大类</label>
          <div class="style-selector">
            <button 
              v-for="major in majorGroupOptions" 
              :key="major"
              class="style-option"
              :class="{ active: selectedMajorGroup === major }"
              @click="selectedMajorGroup = major"
            >
              {{ major }}
            </button>
          </div>
        ...
        <div class="selector-group">
          <label class="selector-label">期望字数规模</label>
          <div class="style-selector">
            <button 
              v-for="words in wordLengthOptions" 
              :key="words"
              class="style-option"
              :class="{ active: selectedWordLength === words }"
              @click="selectedWordLength = words"
            >
              {{ words }}
            </button>
          </div>
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          <label class="selector-label">输入实践背景、实验过程或实习心得要点</label>
          <div style="display: flex; gap: 0.5rem;">
            <button v-if="userInput" class="text-link-btn" @click="userInput = ''">清空输入</button>
            <button class="text-link-btn" @click="showOutlineModal = true">标准报告排版提纲</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
          placeholder="请简要描述您的实践项目、实验步骤或岗位实习内容...（例如：暑期赴山区开展特色农产品直播电商调研，围绕农户意愿、物流痛点、品牌包装及后续帮扶对策撰写 3000 字社会实践报告。）"
          style="min-height: 130px;"
        ></textarea>
        <div style="display: flex; justify-content: space-between; font-size: 0.75rem; color: var(--text-secondary);">
          <span>字符数: {{ userInput.length }} 字</span>
          <span>建议包含实践/实习名称、开展时间、核心步骤与心得感悟</span>
        </div>
      </div>
      ...
      <div style="display: flex; gap: 0.75rem;">
        ...
        >
          {{ loading ? '正在严谨生成规范报告方案与总结中...' : '开始生成实践与课堂报告' }}
        </button>
    ...
    <section v-if="result || loading" class="glass-card">
      <div class="result-header">
        <span class="result-title">实践与课堂报告指导方案</span>
        <div class="button-actions">
          <button v-if="result" class="icon-btn" @click="copyText">
            {{ copied ? '已复制报告全文' : '复制报告内容' }}
          </button>
    ...
    <section v-if="showHistory" class="glass-card" style="margin-top: 1rem;">
      <div class="result-header">
        <span class="result-title">本地报告历史记录</span>
      ...
      <div v-else class="history-grid" style="display: flex; flex-direction: column; gap: 0.75rem; max-height: 320px; overflow-y: auto;">
        <div v-for="item in historyList" :key="item.id" class="history-item" style="padding: 1rem; background: rgba(0,0,0,0.2); border-radius: 10px; border: 1px solid var(--card-border);">
          <div style="display: flex; justify-content: space-between; font-size: 0.8rem; color: var(--text-secondary); margin-bottom: 0.4rem;">
            <span>{{ item.timestamp }} · [{{ item.reportType }} / {{ item.majorGroup }}]</span>
    ...
    <div v-if="showOutlineModal" class="modal-overlay" @click.self="showOutlineModal = false">
      <div class="modal-content" style="max-width: 480px;">
        ...
        <p style="text-align: left; font-size: 0.825rem; margin-bottom: 1rem; color: var(--text-secondary);">
          规范的高校报告应包含以下清晰的学术与实践结构段落：
        </p>
        <div class="modal-scroll-area" style="text-align: left; font-size: 0.825rem;">
          <div v-for="(sec, idx) in outlineSections" :key="idx" style="margin-bottom: 0.75rem; padding: 0.5rem 0.75rem; background: rgba(255,255,255,0.03); border-radius: 8px; border: 1px solid rgba(255,255,255,0.05);">
            <div style="color: var(--accent-color); font-weight: bold; margin-bottom: 0.2rem;">{{ sec.title }}</div>
            <div style="color: var(--text-primary); margin-bottom: 0.2rem;">核心要点: {{ sec.keyPoints }}</div>
            <div style="color: var(--text-secondary); font-size: 0.775rem;">写作建议: {{ sec.advice }}</div>
          </div>
        </div>
        <button class="modal-btn" style="margin-top: 1rem;" @click="showOutlineModal = false">关闭</button>
      </div>
    </div>
    ...
    <div v-if="showShareGuide" class="modal-overlay" @click.self="showShareGuide = false">
      <div class="modal-content">
        <h3>分享实践与课堂报告工具</h3>
        <p>扫码关注或将链接转发给同专业同学，高效完成实践、实训与实习报告写作。</p>
    ...
    <div v-if="showPrivacy" class="modal-overlay" @click.self="showPrivacy = false">
      <div class="modal-content">
        ...
        <div class="modal-text-content modal-scroll-area">
          <p>我们重视您的学术与实习隐私。您在本工具中输入的实践背景与实习记录仅用于实时大模型生成，系统不会在云端存储您的私密信息。</p>
    ...
    <div v-if="showTerms" class="modal-overlay" @click.self="showTerms = false">
      <div class="modal-content">
        ...
        <div class="modal-text-content modal-scroll-area">
          <p>欢迎使用网腾无限 AI 实践与课堂报告生成专家。本工具生成的报告建议与框架供学习、实训总结与教务提交参考。</p>
          <p>生成内容由大语言模型实时撰写，请结合您实际参与的实践项目细节与指导教师要求进行微调。</p>
        </div>
...
// 配置参数
const appTitle = ref(appConfig.title || '网腾无限AI - 实践与课堂报告生成专家');
...
const reportTypeOptions = [
  { label: '社会实践与调研报告', value: '社会实践与调研报告' },
  { label: '专业实训与实验报告', value: '专业实训与实验报告' },
  { label: '毕业实习与岗位总结', value: '毕业实习与岗位总结' },
  { label: '课堂学术研讨与心得', value: '课堂学术研讨与心得' }
];
const activeReportType = ref(reportTypeOptions[0].value);

// 专业大类与字数
const majorGroupOptions = ['理工农医类', '人文社科类', '经济管理类', '艺术设计类', '师范教育类'];
const selectedMajorGroup = ref('理工农医类');

const wordLengthOptions = ['1500字', '3000字', '5000字'];
const selectedWordLength = ref('3000字');

// 评估指标列表
const metricsList = [
  { key: 'practicalRelevance', label: '实践契合度' },
  { key: 'academicDepth', label: '学术思考深度' },
  { key: 'structureCompleteness', label: '结构完整度' },
  { key: 'reflectionInsight', label: '心得体会感悟度' },
  { key: 'formatNorm', label: '报告格式规范度' }
];
...
const outlineSections = [
  { title: '一、实践/课题背景', keyPoints: '实践目的、团队分工、指导老师与课题意义', advice: '突出实践的现实契合度与专业相关性' },
  { title: '二、实践过程与方法', keyPoints: '具体步骤、技术路线、调查问卷/实验工具', advice: '结合真实数据、案例或流程图展开叙述' },
  { title: '三、成果与建议', keyPoints: '产出成果、发现的问题与可行性对策提案', advice: '论据扎实，建议具备可操作性' },
  { title: '四、心得体会', keyPoints: '理论联系实际感悟、团队协同与职业规划升华', advice: '感情真挚，展现个人思想与专业素养提升' }
];

// 计算纯结果文本 (剔除打分标签 [BAOGAO_SCORES])
const displayResultText = computed(() => {
  if (!result.value) return '';
  return result.value.replace(/\[BAOGAO_SCORES\][\s\S]*?\[\/BAOGAO_SCORES\]/g, '').trim();
});

// 解析打分标签
const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[BAOGAO_SCORES\](.*?)\[\/BAOGAO_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('baogao_history_records');
...
const saveHistory = () => {
  localStorage.setItem('baogao_history_records', JSON.stringify(historyList.value));
};
...
const addHistoryRecord = () => {
  const newItem: HistoryItem = {
    ...
    majorGroup: selectedMajorGroup.value,
...
const applyHistory = (item: HistoryItem) => {
  ...
  activeReportType.value = item.reportType;
  selectedMajorGroup.value = item.majorGroup;
...
const handleGenerate = async () => {
  ...
  try {
    const response = await fetch(apiEndpoint, {
      ...
      body: JSON.stringify({
        taskType: 'text',
        prompt: `任务指导: ${promptTopic.value}\n【报告类型】: ${activeReportType.value}\n【专业大类】: ${selectedMajorGroup.value}\n【字数规模】: ${selectedWordLength.value}\n【实践/课题/实验描述】: ${userInput.value}`,
        style: activeReportType.value
      })
    });
...
const handleApplyTemplate = (payload: { prompt: string; reportType?: string; majorGroup?: string }) => {
  userInput.value = payload.prompt;
  if (payload.reportType) activeReportType.value = payload.reportType;
  if (payload.majorGroup) selectedMajorGroup.value = payload.majorGroup;
  if (inputCardRef.value) {
    inputCardRef.value.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }
};
