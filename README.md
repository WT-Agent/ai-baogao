...
**一个基于 Vue 3 + Vite + Vanilla CSS 构建的高效 AI 行业调研与分析报告生成微应用，具备深色玻璃拟态自适应交互与微信端 H5 体验**
...
1. **报告主题与课题背景构建**：明确具体实践与调研目的、理论依据、实施时间地点及岗位与项目职责定位。
2. **实践过程与实验数据记录**：清晰记录实践或实验的具体推进步骤、关键数据、方法技术及遇到问题的解决过程。
3. **成果提炼与问题改进建议**：归纳提炼实践产生的重要学术与社会成果，深入剖析不足之处并提出后续改进对策。
4. **心得体会与思想收获提升**：结合专业理论深入阐述实践中的认知升华、团队协作体会与个人职业发展感悟。

## AI 评估指标

1. **实践相关性 (practicalRelevance)**：评估报告内容与实际课题、岗位职责及调研背景的契合程度。
2. **学术深度 (academicDepth)**：评估报告在理论依据、分析方法及专业学术表达上的深度与严谨性。
3. **结构完整度 (structureCompleteness)**：评估报告逻辑框架、模块划分及段落推进的系统性与完整性。
4. **反思洞察力 (reflectionInsight)**：评估报告对实践不足、改进对策及个人心得升华的洞察水平。
5. **格式规范度 (formatNorm)**：评估报告格式、语言表达规范及标准化文书排版水平。
...
node bin/cli.js set ai-baogao prompt "你是一位高校教务处指导教师、优秀实践项目评审专家兼学术写作导师..."
